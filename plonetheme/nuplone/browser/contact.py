...
class IContact(form.Schema):
    ...
    form.widget(name="plonetheme.nuplone.browser.contact.TextSpan7")

    email = schema.ASCIILine(
        title=_("label_email", default="Email address"), required=True
    )
    form.widget(email="plonetheme.nuplone.browser.contact.TextSpan7")

    subject = schema.TextLine(
        title=_(u"label_subject", default=u"Subject"), required=True
    )
    form.widget(subject="plonetheme.nuplone.browser.contact.TextSpan7")

    message = schema.Text(
        title=_("label_contact_text", default="Your message"), required=True
    )
    form.widget(message="plonetheme.nuplone.browser.contact.TextLines4Rows")
