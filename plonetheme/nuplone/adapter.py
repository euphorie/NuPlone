...
class StatusMessage(adapter.StatusMessage):
    ...
    def addHTML(self, text, type=u"info"):
        ...
        old = annotations.get(HTMLMESSAGEKEY, context.cookies.get(HTMLMESSAGEKEY))
    ...
    def show(self):
        ...
        context = self.context
        annotations = IAnnotations(context)
        msgs = annotations.get(STATUSMESSAGEKEY, context.cookies.get(STATUSMESSAGEKEY))
        msgs = msgs and adapter._decodeCookieValue(msgs) or []

        html_msgs = annotations.get(HTMLMESSAGEKEY, context.cookies.get(HTMLMESSAGEKEY))
